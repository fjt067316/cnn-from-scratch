...
int model(){
    ...
    model->add_conv_layer(8, 1, 3, 0.0005); // 1x28x28 -> 8x24x24
    ...
    // cout << "hit" << endl;
        ...
        num_correct += model->forward(image_1x28x28, image_label);
...
//         row.erase();
//         getline(inputFile, row);
//         Tensor<double> input = decodeCsvString(row); // input = (784)
...
//     ConvolutionLayer* conv_8C3 = new ConvolutionLayer(8, 1, 3, 0.0001); // 8x26x26
//     BatchNorm3D* bn = new BatchNorm3D(8, 0.0001);
//     Pool* max_pool = new Pool(2,2);
//     FullyConnectedLayer* fc_1352_10 = new FullyConnectedLayer(8*13*13, 10, 0.0001, true);
//     Softmax* softmax = new Softmax();
...
//         Tensor<double> image_1x28x28 = reshape_input(input, 28, 28); // image_1x28x28: 1x28x28
//         Tensor<double> conv_8C3_out = conv_8C3->forward(image_1x28x28); // conv_8C3_1_out: 8x26x26
//         // dropout3d(0.4, conv_8C3_out);
//         Tensor<double> conv_8C3_bn_out = bn->forward(conv_8C3_out);
//         Tensor<double> max_pool_out = max_pool->forward(conv_8C3_bn_out); // conv_8C3_2_out: 8x24x24
//         // Tensor<double> flattened_pool = flatten(max_pool_out); // flattened_conv_64C3: 1024
        
//         Tensor<double> fc_1352_10_out = fc_1352_10->forward(flattened_pool); // fcl_1024_128: 128x
//         Tensor<double> outputs = softmax->forward(fc_1352_10_out);
//         Tensor<double> dLdZ(outputs.size);

//         pair<int, double> prediction = get_pred(outputs);
//         int pred_idx = prediction.first;
//         double pred = prediction.second;
...
//         num_correct += (pred_idx == image_label) ? 1 : 0;
//         loss += -log(outputs[image_label]);
//         // cout << i << ", " << bool(image_label==idx) << ", " << outputs[image_label] << endl;
//         // copy(outputs.begin(), outputs.end(), dLdZ.begin()); // copy coutputs to dLdZ
//         // dLdZ[image_label]--; // substract one hot encoded label for initial dL/dZ
//         dLdZ[image_label] = -1 / outputs[image_label];
//         // printArray(dLdZ, 10);
//         Tensor<double> dLdZ_softmax = softmax->backwards(dLdZ);
//         // printArray(dLdZ_softmax, 10);
//         Tensor<double> dLdZ_1352 = fc_1352_10->backwards(dLdZ_softmax);
//         // printArray(dLdZ_1352, 10);
//         // print_vector(fc_1352_10->weights);
//         Tensor<double> dLdZ_138 = reshape(dLdZ_1352, 8, 13, 13); // dLdZ_138: 8x13x13

//         Tensor<double> dLdZ_826 = max_pool->backwards(dLdZ_138); // dLdZ_1612: 8x26x26
//         Tensor<double> dLdZ_826_bn = bn->backwards(dLdZ_826);
//         // printArray(dLdZ_826[0][0], 10);
//         Tensor<double> dLdZ_final = conv_8C3->backwards(dLdZ_826_bn); //dLdZ_final: 1x28x28
