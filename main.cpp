...
int simple() { 
    ...
    for (int i = 0; i < n; i++) {
        ...
        bn->batch_normalize(&conv_64C5_out);
        ...
        vector<double> dLdZ_128 = fc_128_10->backwards(dLdZ_softmax, learning_rate);
        vector<double> dLdZ_1024 = fc_1024_128->backwards(dLdZ_128, learning_rate);
        ...
        bn->backwards(&conv_64C5_out, learning_rate);
        vector<vector<vector<double>>> dLdZ_3212 = conv_64C5->backwards(dLdZ_6488, learning_rate); //dLdZ_3212: 32x12x12
        vector<vector<vector<double>>> dLdZ_3224 = max_pool_32->upsample(dLdZ_3212); // dLdZ_3224:  32x24x24
        vector<vector<vector<double>>> dLdZ_final = conv_32C5->backwards(dLdZ_3224, learning_rate); //dLdZ_final: 1x28x28
...
int custom() {
    ...
    // double eps = 1e-8;    // small value to avoid division by zero
    // Define vectors for the first and second moments
    ...
    for (int i = 0; i < n; i++) {
        ...
        vector<double> dLdZ_softmax = softmax->backwards(dLdZ);
        // printArray(dLdZ_softmax, 10);
        vector<double> dLdZ_256 = fc_256_10->backwards(dLdZ_softmax, new_learning_rate);
        // dLdZ_256 = bn_1d->backwards(dLdZ_256, learning_rate);
        vector<double> dLdZ_1352 = fc_7744_256->backwards(dLdZ_256, new_learning_rate);
        // printArray(dLdZ_1352, 10);
        // print_vector(fc_1352_10->weights);
        vector<vector<vector<double>>> dLdZ_822 = reshape(dLdZ_1352, 16, 22, 22); // dLdZ_138: 8x13x13
        // bn_16->backwards(&dLdZ_822, learning_rate);
        vector<vector<vector<double>>> dLdZ_16 = conv_16C5->backwards(dLdZ_822, new_learning_rate); //dLdZ_final: 1x28x28
        // bn_8->backwards(&dLdZ_16, learning_rate);
        vector<vector<vector<double>>> dLdZ_final = conv_8C3->backwards(dLdZ_16, new_learning_rate); //dLdZ_final: 1x28x28
    }
    return 0;
